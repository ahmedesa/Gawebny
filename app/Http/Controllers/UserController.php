...
class UserController extends Controller
{
	...
	public function savedQuestion($id)
	{
		...
		if(request()->order){
			if(request()->order == 'recnet') 
				$saved_questions = Question::whereHas('savedq', function($q) use ($id) 
					{ $q->where('user_id','=', $id); })->orderBy('created_at' ,'desc')->paginate(5);
			if(request()->order == 'votes')   
				$saved_questions = Question::whereHas('savedq', function($q) use ($id) 
					{ $q->where('user_id','=', $id); })->orderBy('votes' ,'desc')->paginate(5);
		}else {
			$saved_questions = Question::whereHas('savedq', function($q) use ($id) 
				{ $q->where('user_id','=', $id); })->paginate(5);		}
